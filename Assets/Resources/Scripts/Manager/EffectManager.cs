...
public class EffectManager : MonoBehaviour
{
    ...
    {
        Effect effect = effects.Find(e => e.effectType == type && !e.GetIsPlaying());

        if (effect == null)
        {
            Effect origin = effects.Find(e => e.effectType == type);
            effect = Instantiate(origin, transform).GetComponent<Effect>();
            effects.Add(effect);
        }
